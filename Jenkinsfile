node {
	...
	stage("Test") {
		...
		sh 'docker rm -f dev_my_app; docker run --name dev_my_app -p 443:80 -dit oriexsol/my_appt:test'
	...
	stage ("Deploy") {
		...
		sh 'docker rm -f my_app_prod; docker run --name my_app_prod -p 80:80 -dit oriexsol/my_app:deploy'
	}
}
